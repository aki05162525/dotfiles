...
- macOS: `home-manager switch --flake .#akihiro@mac`
...
通常は修正不要。ユーザー名が `akihiro` 以外、あるいは Intel Mac など新しいホストを
足す場合だけ `flake.nix` を編集する:

```nix
homeConfigurations = {
  "akihiro@wsl" = mkHome { system = "x86_64-linux";  homeDirectory = "/home/akihiro"; };
  "akihiro@mac" = mkHome { system = "aarch64-darwin"; homeDirectory = "/Users/akihiro"; };
  # 例: Intel Mac を足すなら
  # "akihiro@mac-intel" = mkHome { system = "x86_64-darwin"; homeDirectory = "/Users/akihiro"; };
};
```
...
リポジトリルートで、マシンに対応する構成を指定する(以下は WSL2 の例。mac は `.#akihiro@mac`):
...
```sh
rm ~/.config/<該当ファイル>
home-manager switch --flake .#akihiro@wsl   # mac は .#akihiro@mac
```
